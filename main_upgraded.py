...
batch_size = 256
train_iter = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, pin_memory=True)
test_iter = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size, shuffle=False, pin_memory=True)
...
def collate(batch):
    ...
    for i in range(batch_size // 2):
        ...
        seq2, label2 = batch[i + batch_size // 2][0], batch[i + batch_size // 2][1]
    ...
    return (
        torch.cat(seq1_ls).to(device),
        torch.cat(seq2_ls).to(device),
        torch.cat(label_ls).to(device),
        torch.cat(label1_ls).to(device),
        torch.cat(label2_ls).to(device)
    )
